...
const Weather = () => {
    ...
    const getTemperatureClass = () => {
        if (avgTemperature < 0) {
            setBg(`wm-${avgTemperature % 10 < 3 ? String(avgTemperature)[0] : String(avgTemperature - 6)[0]}0`);
        } else if (avgTemperature > 0) {
            setBg(`wp-${avgTemperature % 10 < 3 ? String(avgTemperature)[0] : String(avgTemperature + 6)[0]}0`);
        } else {
            setBg("w-0");
        }
    }
